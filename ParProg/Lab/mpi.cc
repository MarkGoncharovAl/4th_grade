  ...
  { /*rank == 0*/
    for (int i = 0; i < curSize; ++i)
      for (int j = 1; j < size; ++j)
        act (MPI_Recv (data[i + (j - 1) * curSize] , JSIZE , MPI_DOUBLE , j , 0 , MPI_COMM_WORLD , MPI_STATUS_IGNORE));
  }
  ...
  for (int i = 1; i < ISIZE; ++i)
  {
    if (rank != 0)
    {
      ...
      for (int j = start_elem; j < start_elem + cur2Size; ++j)
        data[i][j] = std::sin (2 * data[i - 1][j - 3]);
      act (MPI_Send (&(data[i][start_elem]) , cur2Size , MPI_DOUBLE , /*dest*/0 , 0 , MPI_COMM_WORLD));
    }
    else /*rank == 0*/
    {
      for (int j = 1; j < size; ++j)
        act (MPI_Recv (&(data[i][3 + (j - 1) * cur2Size]) , cur2Size , MPI_DOUBLE , j , 0 , MPI_COMM_WORLD , MPI_STATUS_IGNORE));
    }
    MPI_Barrier (MPI_COMM_WORLD); /* IMPORTANT */
