...
class Object {
  struct Pixel {
    ...
    void r(int color) { data_[0] = color; }
    void g(int color) { data_[1] = color; }
    void b(int color) { data_[2] = color; }
    void t(int color) { data_[3] = color; } // transparent
...
public:
  Object() : data_(pixels_.data()->data()->data_.data()) {
  ...
  void update(int x, int y, short cur_heat) { pixels_[x][y].heat(cur_heat); }
...
int initWindow() {
  ...
  if (!font.loadFromFile(
          "/home/mark/Programming/7/ParProga/MIPT/Compiler/FreeSans-LrmZ.ttf"))
    return 1;
  for (auto &text : axis) {
    text.setFont(font);
